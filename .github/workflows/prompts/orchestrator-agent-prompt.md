...
## Final
  ...
  > **WRITE-RESTRICTION — ORCHESTRATOR ONLY:** Only the orchestrator (you) may write to the knowledge graph. Subagents MUST NOT call `create_entities`, `add_observations`, `create_relations`, `delete_entities`, `delete_observations`, or `delete_relations`. Concurrent subagent writes cause file corruption. Subagents may call `read_graph`, `search_nodes`, and `open_nodes` (read-only).
